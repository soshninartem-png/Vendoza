...
class PromoCode(models.Model):
    ...
    def calculate_discount(self, order_amount, delivery_cost=0):
        ...
        if order_amount < self.minimum_order_amount:
            return 0, False, f"Минимальная сумма заказа: ${self.minimum_order_amount}"
        ...
        if self.discount_type == 'percentage':
            ...
            # БЕСПЛАТНАЯ ДОСТАВКА
            free_shipping = True
            discount_amount = 0  # Скидка на товары = 0, доставка станет бесплатной
            message = "Бесплатная доставка"
        
        else:
            message = "Неверный тип скидки"
        
        return float(discount_amount), free_shipping, message
